...
export function cleanPilot(p){
 ...
 const out={};
 if(p.layout){const l=p.layout;out.layout={group:String(l.group||'').replace(/[^\w-]/g,'').slice(0,40),column:num(l.column,0,1,0)|0,ratio:num(l.ratio,25,75,60),reverse:!!l.reverse,mode:choice(l.mode,['columns','overlay'],'columns'),position:choice(l.position,['left','center','right'],'right'),overlap:num(l.overlap,0,160,64),cardWidth:num(l.cardWidth,25,90,40),padding:num(l.padding,0,64,24),radius:num(l.radius,0,40,16),shadow:l.shadow!==false,cardColor:/^#[0-9a-f]{6}$/i.test(l.cardColor||'')?l.cardColor:'#ffffff',placement:choice(l.placement,['inside','edge'],'edge')};}
