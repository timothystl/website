...
own section by that name), and **Phase D** (the client-side takeover/fallback
JavaScript is simplified down to one rendering path — DONE, 2026-09-07, see
CLAUDE.md's "The takeover mechanism is simplified" section; the blocker
recorded above as "once the remaining handful of pages are published" turned
out to already be cleared by the time Phase D was picked up — every row in
