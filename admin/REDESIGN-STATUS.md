...
| Email templates | `admin/email.js` | **Done** (Task 16) — swept onto the Foundations colours. The earlier `n/a` said these have "their own palette on purpose"; they did not. Those were the PRE-redesign values, never swept. Colour only: no CSS variables (half the clients drop them), font fallbacks kept, and the email geometry — 6px radii, the 3px note accent, the 14px header radius — left alone, because Task 1's radius audit and its drop-the-stripe rule are browser rules that do not travel to an inbox. |
| `admin/scheduler.html` | `admin/scheduler.html` | **`n/a`** — dead code behind the session gate (the only endpoint it talks to does not exist). Its colours were swept with Task 16 anyway, so the definition-of-done grep over `admin/` can return clean; nothing else about it changed. |
