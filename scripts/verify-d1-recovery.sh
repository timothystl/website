...
# Ported from chms's scripts/verify-d1-recovery.sh (export -> disposable restore -> reconcile ->
# delete), adapted for Website's tlc-newsletter-db. Steps [1]-[5] and [7] are unchanged in
# substance. Step [6] (monetary control totals) is the one real content difference: chms's
# schema has genuine dollar-amount columns throughout; tlc-newsletter-db's give_funds/
# give_amount_tiers tables are the only obviously monetary ones here, and this repo has never
# run this drill before to confirm the same keyword scan finds them. So step 6 below runs the
# identical scan but treats zero matches as "not applicable to this schema" rather than a hard
# failure -- do not read a `monetary_controls_matched: 0` result as this schema having no
# money-shaped data; it means the naming-keyword heuristic didn't recognize any column as one,
# which is worth a human look the first time this actually runs, not an assumption either way.
...
# This repo has no package.json/npm lockfile (unlike chms), so there is no local `npx wrangler`
# to resolve against -- the workflow installs Wrangler globally instead (same as deploy.yml
# already does for every other Wrangler step in this repo).
wrangler() {
...
echo "[6/7] Reconciling numeric monetary-looking control totals (if this schema has any)"
...
while IFS=$'\t' read -r table column column_type; do
  ...
  if [[ "$lower_column" =~ (amount|total|balance|fee|cost|budget|pledge|gift|donation|income|expense|tuition|payment|salary|compensation|revenue|principal|interest|allocation|forecast|reserve|price|tier) ]] &&
...
if [[ "$monetary_controls" -gt 0 ]]; then
  ...
  echo "      No column matched the monetary-keyword heuristic on this schema -- skipped, not failed."
  monetary_sha="n/a"
fi
