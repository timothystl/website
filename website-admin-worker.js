...
import { VERSION, html, jsonResponse, sidebarShell, loginPage, setupPage, forgotPasswordPage, resetPasswordPage, permissionCheckboxes, formatDate, escapeHtml, tinymcePageSection, ADMIN_SHELL_CSS, ADMIN_SHELL_JS, SERVICE_WORKER_JS } from './admin/helpers.js';
...
import { buildCalendarFeed, buildIcs, parseCalendarIds, monthRange, shiftMonth,
         ...
         DEFAULT_CATEGORIES, NEUTRAL_CATEGORY,
...
import { migrateLegacyPage, sanitizeBlocks, sanitizeBlock, parseBlocks, newBlock,
         ...
         STAMP_PRESETS, safeUrl, esc as escBlock,
         TEMPLATES, wrapTemplate, BLOCK_CSS, BLOCK_CSS_RAW, blockCssLink, cleanText,
         isSafeObjectPosition, safeZoomFactor, sanitizeClassicRich } from './admin/blocks.js';
...
import { orderPages, filterPages, pageStatus, slugify, uniqueSlug, pageRename,
         withShortLinks, shortLinkFor, shortLinkRoutes, outboundUrl, canReseed,
         handlePagesRoutes } from './admin/pages.js';
import { handleMinistriesRoutes } from './admin/ministries.js';
import {
  NATIVE_FORM_REQUIRED_TYPE, missingNativeForm, sharedEditorApi, pageLayoutContext,
  editorPageData, bustEditorPageData, EDITOR_PAGE_DATA_PREFIXES, NEWS_ORDER_SQL, NEWS_WHERE_SQL,
  richOut, pageData, promoteScheduledPages,
} from './admin/editor-shared.js';
...
import { PALETTE as CHROME_PALETTE, BAR_KEYS, DEFAULTS as CHROME_DEFAULTS,
         appearanceFromForm, sanitizeAppearance,
...
// jsonResponse moved to admin/helpers.js (imported below) — pure, zero
// dependencies, used across every admin domain, not just the page editors.
...
// EDITOR_HEADERS / pageSettings / pageEditors / pageEditorStatus moved into
// admin/pages.js (imported below) -- they were used exclusively by the Site
// Pages admin routes, now in handlePagesRoutes there.

// cleanSlug / oversizeImage / pageLayoutContext / sharedEditorApi / the
// editorPageData caching layer / NEWS_ORDER_SQL / NEWS_WHERE_SQL / richOut /
// pageData / promoteScheduledPages / missingNativeForm /
// NATIVE_FORM_REQUIRED_TYPE moved into admin/editor-shared.js (imported
// below) -- the block editor's scaffolding shared by Site Pages and
// Ministries, plus the self-filling-block data bundle every page (admin and
// public) renders with. See that file's own header comment for why `ctx`,
// `linkTargets`, and `MINISTRY_EDITOR_HTML` stay here instead.
...
export default {
  ...
  async _fetch(request, env, ctx) {
    ...
    const SCHEMA_VERSION = '2026-09-24-functional-fixes'; // Atomic gym overlaps and serialized newsletter campaign operations.
...
${PAYROLL_HTML}`, 'Payroll');
    ...
    // was -- not one upfront check. ctx is passed directly (not a shared bag,
    // unlike Site Pages/Ministries) for the two background sends this domain
    // itself owns.
    {
      const looksLikeNewsletter = path === '/new' || path === '/publish' ||
        path.startsWith('/newsletter/') || path.startsWith('/edit/') || path.startsWith('/send-email/') ||
        path.startsWith('/schedule-email/') || path.startsWith('/delete/') ||
        path.startsWith('/newsitems') || path.startsWith('/christian-education');
      const r = await handleNewsletterRoutes(request, env, path, method, currentUser, url, ctx,
        looksLikeNewsletter ? await pageBadges() : {});
      if (r) return r;
    }

    // ════════════════════════════════════════════════════════
    // ── SITE PAGES & MINISTRIES (admin/pages.js, admin/ministries.js) ──
    // Site Pages: the all-pages list, drawers, Church details, and the page
    // editor's own API. Ministries: the ministry list, the retired old block
    // editor's compat shims, ministry metadata, and per-ministry posts.
    // Both gated on their own permissions inside their handlers.
    // editorShared carries the handful of things these two domains share
    // with each other, with the public /api/pages handler, and with the
    // daily cron. Most of it now lives in admin/editor-shared.js (imported
    // above); this stays a parameter bag rather than each of those importing
    // the shared module directly because two of its members are worker-only:
    // `ctx` (request-scoped) and `linkTargets` (a per-request-memoized
    // closure over `env`) -- see admin/editor-shared.js's own header
    // comment for the full reasoning.
    {
      const editorShared = {
        sharedEditorApi, editorPageData, pageLayoutContext, linkTargets,
        promoteScheduledPages, missingNativeForm, NATIVE_FORM_REQUIRED_TYPE,
        MINISTRY_EDITOR_HTML, ctx,
      };
      const pagesR = await handlePagesRoutes(request, env, path, method, currentUser, url,
        (path === '/pages' || path.startsWith('/pages/')) ? await pageBadges() : {}, editorShared);
      if (pagesR) return pagesR;

      const ministriesR = await handleMinistriesRoutes(request, env, path, method, currentUser, url,
        (path.startsWith('/ministries') || path === '/youth' || path.startsWith('/youth/')) ? await pageBadges() : {}, editorShared);
      if (ministriesR) return ministriesR;
    }
