...
- **`estimatedCardFeeCents(row, settings)`** in `admin/market.js` — `amount_due_cents` minus `tables × settings.tableFee`, which is the gap between what a card row was actually asked for and the flat table fee, whatever the card-fee percentage was assumed to be the day this vendor applied. ⚠ **Deliberately not `priceBreakdown()` recomputed against today's settings** — that would let the office changing the fee percentage or the table price mid-season retroactively move every past vendor's estimate. Reading it back out of the row's own recorded `amount_due_cents` survives that.
- **⚠ NEVER WRITTEN TO `square_fee_cents`.** That column means "Square told us this," and blurring a guess into it would let the office reconcile a real bank deposit against a number nobody ever confirmed — exactly the distinction `squareFeeCents()`'s own null-vs-zero rule (see the section above) exists to protect. This is computed fresh at render time only, worded "Card fee, estimated at the standard rate — about $X" rather than "Square kept $X", and a real, reported figure always wins over it the moment one exists.
- **Only for a paid card row.** Check/cash never had a card fee to estimate; an unpaid, waived, or dropped row has no payment to estimate a fee against at all.

Run: the `estimatedCardFeeCents` group in `node admin/market.test.mjs` (374, including that a changed fee percentage since a vendor applied does not move their estimate), and a new group in `test/admin-redesign.test.mjs` (1774) that marks a row paid with no `square_fee_cents`, confirms the estimate renders and is never worded like the real figure, then sets a real `square_fee_cents` and confirms it replaces the estimate rather than the two showing together.

### A vendor's own answers can be corrected, how they are paying can be switched, and the Square webhook reports what it actually kept (v5.57.0, 2026-09-03)

Dinger, on the vendor row's Contact section: *"i want to be able to edit what they submit, like their name."* Then, days later, with a real card row that had ended up paid by check instead: *"i had someone pay offline by check, can you give me option to record it that way."* And, separately: *"i want this payment system recording from square to get a webhook back that shows how much actually gets deposited after the fees are collected. for example we are collecting 35.14 on the one table charge out of the 36.50 that they pay."*

**A vendor's own answers were read-only.** The Contact section of the expanded row showed what was typed on the public application and nothing else — a misspelled name or a mistyped email had no fix short of deleting the application and asking the vendor to reapply.

- **Business name, contact person, email and phone are editable in the row now**, saving one field at a time through the same `/market/update` route and field allowlist every other cell already uses — `field=participant_names` writes `contact_name` directly, `field=business_name` merges into `fields_json` the way `category` already does, `email` is validated with the same regex the public application uses and lower-cased on the way in, and none of the four is required the way it is on that public form: a correction is not a resubmission, and clearing a box by mistake must not block saving the other field the coordinator actually meant to fix.
- **`vendorDisplayName(row)`** is the one place a row becomes a name — `business_name || participant_names || '(no name given)'`, and the byline underneath it — shared by the row's own markup and the JSON a save answers with, so a corrected name redraws in the row immediately rather than waiting on a reload.
- The whole-form (no-script) fallback carries the same four fields, since the expanded panel is a real `<form>`.

**Paying by card or by check/cash was fixed at application time, and a vendor sometimes ends up paying the other way** — an envelope left with the office, cash handed over at the door, after applying online. The static "Paying by card, online" text is a `<select>` now (`field=payment_method`), and switching it **re-prices the table** the same way changing the table count already does: check or cash is the flat table fee, card is the grossed-up total with the processing fee folded in — computed fresh from `priceBreakdown()`, never touching an amount already recorded as paid, which is a fact about what arrived, not a re-guess of what should have been asked.

- **⚠ The check column had never needed to change SHAPE before, and now it does.** A vendor switched to check has to grow the check-number/date inputs (or lose them, switching back) without a page reload. `admin/ui.js`'s `repaint()` tracks the column's current shape on its own `data-checkmode` attribute — the same box, class `tlc-mkt-quiet`, renders two different sentences ("Card, online" vs. "nothing owed"), so the DOM alone cannot say which mode is showing — and `buildCheckCol()` rebuilds it from scratch only when that attribute disagrees with what the server just said, built with the DOM API and `textContent`/`.value` rather than an HTML string so nothing coming back from the server ever needs escaping on the way in. A number the coordinator is mid-typing into the pending inputs is never touched, because a save that leaves the mode at `pending` takes the narrower, existing path instead.

**The Square webhook only ever recorded what the vendor was CHARGED, never what the church actually NETS.** `payment.amount_money.amount` is the gross card charge; Square's own `Payment.processing_fee` array is what the card network and Square actually took — which can differ a few cents from the flat 2.9% + 30¢ `priceBreakdown()` assumes, depending on the card's own network and brand (debit vs. credit, a rewards card). That gap is exactly the difference between what a vendor paid and what shows up in the bank account, and there was no way to see it.

- **`squareFeeCents(payment)`** in `admin/square.js` sums the array. ⚠ **Returns `null`, not `0`, when the array is absent or empty** — Square does not always know the fee at the instant a payment completes; it can be calculated and reported on a *later* `payment.updated` redelivery for the same payment. Null means "not reported yet"; a real, reported $0 fee is a different fact and has to stay distinguishable from "we don't know."
- **`site_event_registrations.square_fee_cents`** is the new column. The webhook route no longer skips a payment just because the row is already `paid` — it now separately checks whether the fee changed, and updates it even on a redelivery that arrives after the mark-paid one already landed. A `null` fee is never written over a real one already recorded, and an unchanged fee is never rewritten (which is what keeps a same-fee redelivery from logging a second audit entry).
- **Shown in the row's Money panel** once both figures exist: *"Square kept $1.36 — $35.14 actually deposited."* — Andrew's own example, byte for byte. And in the CSV export, as two new columns (Square fee, Net deposited), blank rather than zero for anything not paid through Square or not yet reported.
- **⚠ Not verified against a live Square account** — same caveat `admin/square.js`'s own header note already carries for the rest of this file: this sandbox has no outbound access to Square's API. The shape of `processing_fee` is Square's documented one; confirm it with Square's Sandbox "Send test event" button before trusting the real figures.

Run: `node admin/square.test.mjs` (22, `squareFeeCents` verified for the null-vs-zero distinction and multi-entry summing), `node admin/market.test.mjs` (368, `vendorCellState`'s `payment_method`, `marketRowFromRegistration`'s `square_fee_cents`), and three new groups in `test/admin-redesign.test.mjs` (1770 total) — a vendor's contact fields corrected through both the single-cell and whole-form paths, a table re-priced by switching payment method while an already-recorded payment survives untouched, and the Square fee landing on a redelivery after the row is already paid, without a third audit entry once the figure stops changing.
...
### Every push notification logs to a Push Log screen (v5.41.0, 2026-08-21)
...
### The vendor row is the form, and the roster is four views (v5.30.0, 2026-08-19)
