...
import { VERSION, html, jsonResponse, sidebarShell, loginPage, setupPage, forgotPasswordPage, resetPasswordPage, permissionCheckboxes, formatDate, escapeHtml, tinymceEditorSection, tinymcePostSection, tinymceYouthSection, tinymcePageSection, tinymcePastorSection, tinymceNoteSection, ADMIN_SHELL_CSS, ADMIN_SHELL_JS, SERVICE_WORKER_JS } from './admin/helpers.js';
...
import { migrateLegacyPage, starterBlocks, sanitizeBlocks, sanitizeBlock, parseBlocks, newBlock,
         ...
         STAMP_PRESETS, safeUrl, esc as escBlock, blocksClientConfig, makeBlockId,
...
${newsEventTimeScript()}`, 'Edit post — TLC Admin', TINYMCE_HEAD);
    ...
    // ════════════════════════════════════════════════════════
    // ── SITE PAGES (admin/pages.js owns the routes) ──────────────
    // The all-pages list, drawers, Church details, and the page editor's own
    // API. Gated on pages_edit/pages_edit_own inside handlePagesRoutes
    // itself. editorShared carries the handful of things this domain still
    // shares with the (not yet extracted) Ministries editor, the public
    // /api/pages handler, and the daily cron -- see admin/pages.js's own
    // comment on handlePagesRoutes for why those stay here for now.
    {
      const editorShared = {
      ...
      const r = await handlePagesRoutes(request, env, path, method, currentUser, url,
